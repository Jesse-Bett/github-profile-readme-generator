...
const DisplayWork = (props) => {
    if(props.prefix && props.project){
        if(props.link) {
            return (<>{`- ${props.prefix} [${props.project}](${props.link})`}<br/></>);
        } else {
            return (<>{`- ${props.prefix} **${props.project}**`}<br/></>);
        }
    }
    if (props.prefix && props.link) {
        return (<>{`- ${props.prefix} [${props.link}](${props.link})`}<br/></>);
    }
    return '';
}
...
const VisitorsBadge = (props) => {
    ...
    if (props.show) {
        return (<>{`<p align="left"> <img src=${link} alt=${props.github}/> </p>`}<br/></>);
    }
    return '';
}
